...
vec2 sdf(vec3 p) {
  vec3 op = p;
  float sz = .6 + volume/80.;// + (volume * float(sin(time/3.)<swt))/10.
  p.x += .05;
  p.xyz = mod(p.xyz + sz/2., sz) - sz/2.;
  float rot_offset = length(texture2D(spectrum, op.xy/20.));
  //rot_offset = 0.;
  p.xy *= rot(time + rot_offset);
  ...
  float r = .07; + volume/200.;1.27 - 1./(length(texture2D(spectrum, mod(floor((op.xz-50.)*sz)+sz/2.,sz)-sz/2.))*.8);
  float circ_field = length(p-.3)-r;
  return vec2(max(circ_field, -(length(op)-10.)), p.y);
}
